...
function updateInfo(year, state) {
    ...
    if (!state) {
        ...
        info.innerHTML = `<p style="font-family: sans-serif"><strong>Context: ${year}</strong> <br> ${context}</p>`;
        console.log(context);
    } else {
        const context = contextStates[state];
        info.innerHTML = `<p style="font-family: sans-serif"><strong>Context: ${allStatesData["2011"][state].stateName}</strong> <br> ${context}</p>`;
        console.log(context);
    }
}
